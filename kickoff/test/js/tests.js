...
partialElement.hide();
var result = populatePartial("firefox", "33.2", previousBuilds);
assert.ok( result );
assert.strictEqual($('#partials').val(), "31.0build2,29.0build2,28.0build2");

partialElement = $('#partials');
var result = populatePartial("firefox", "33.2b2", previousBuilds);
assert.ok( result );
assert.strictEqual($('#partials').val(), "28.0b5build2,27.0b5build2");

partialElement = $('#partials');
var result = populatePartial("firefox", "24.2.0esr", previousBuilds);
assert.ok( result );
// Empty on purpose. 31.0.2esr is less recent
assert.strictEqual($('#partials').val(), "");

partialElement = $('#partials');
var result = populatePartial("firefox", "31.2.0esr", previousBuilds);
assert.ok( result );
...
partialElement = $('#partials');
var result = populatePartial("thunderbird", "31.2.0", previousBuilds);
assert.ok( result );
