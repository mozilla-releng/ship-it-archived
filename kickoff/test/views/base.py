...
class ViewTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with app.test_request_context():
            db.init_app(app)
            db.create_all()
            r = FennecRelease('joe', '1', 1, 'a', 'abc', 'http://foo', True)
            r.ready = True
            db.session.add(r)
            r = FennecRelease('joe', '4', 4, 'a', 'abc', 'http://foo', True)
            db.session.add(r)
            r = FennecRelease('joe', '4', 5, 'a', 'abc', 'http://foo', False)
            db.session.add(r)
            r = FirefoxRelease('0,1', 'joe', '2', 1, 'a', 'def', 'http://bar', True)
            r.complete = True
            r.ready = True
            db.session.add(r)
            r = ThunderbirdRelease('ghi', '0', 'bob', '2', 2, 'b', 'ghi', 'http://baz', True)
            r.complete = True
            r.ready = True
            db.session.add(r)
            db.session.commit()
