...
if [ ! -e /app/.cache/mysql/db.done ]; then
    ...
    # the sample data is for schema version 17
    PYTHONPATH=${PYTHONPATH}:/app/vendor/lib/python python /app/migrate_repo/manage.py upgrade --version=17 mysql://shipit:shipitpw@shipitdb/shipit migrate_repo
